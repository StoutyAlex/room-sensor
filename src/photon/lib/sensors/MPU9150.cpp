...
boolean MPU9150::begin(int addrI2C)
{
...
int MPU9150::readSensor(int addrI2C, int addrL, int addrH){
	Wire.beginTransmission(addrI2C);
	Wire.write(addrL);
...
int MPU9150::readSensor(int addrI2C, int addr){
	Wire.beginTransmission(addrI2C);
	Wire.write(addr);
	Wire.endTransmission(false);
...
int MPU9150::writeSensor(int addrI2C, int addr, int data){
	Wire.beginTransmission(addrI2C);
	Wire.write(addr);
	Wire.write(data);
