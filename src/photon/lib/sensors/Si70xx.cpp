...
float Si70xx::readTemperature(void)
{
	float temp;
	int32_t rawTemp;
...
float Si70xx::readHumidity(void)
{
	float humi;
	int32_t rawHumi;
...
uint16_t Si70xx::read16(uint8_t addr)
{
	uint16_t ret;
	Wire.beginTransmission(_addr);
	Wire.write(addr);
...
void Si70xx::write8(uint8_t addr)
{
	Wire.beginTransmission(_addr);
	Wire.write(addr);
	Wire.endTransmission();
